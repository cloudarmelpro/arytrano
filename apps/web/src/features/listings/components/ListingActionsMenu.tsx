...
import { cloneListingAction } from '../actions/clone-listing'
      ...
      <Menu.Portal>
        ...
        >
          ...
          <Menu.Popup className="min-w-[11rem] overflow-hidden rounded-xl bg-popover p-1 text-sm text-popover-foreground shadow-lg ring-1 ring-foreground/10 outline-none data-open:animate-in data-open:fade-in-0 data-open:zoom-in-95 data-closed:animate-out data-closed:fade-out-0">
            ...
            {/* OWN-06 — clone into a fresh DRAFT and jump to its editor. */}
            <Menu.Item
              disabled={pending}
              onClick={() => {
                startTransition(async () => {
                  const result = await cloneListingAction(listingId)
                  if (!result.ok && result.message) toast.error(result.message)
                })
              }}
              className="group/item flex cursor-pointer items-center gap-2.5 rounded-md px-3 py-2.5 text-foreground outline-none transition data-disabled:cursor-not-allowed data-disabled:opacity-50 data-highlighted:bg-primary/10"
            >
              <span className="flex h-4 w-4 items-center justify-center text-muted-foreground transition group-data-highlighted/item:text-primary">
                <IconClone />
              </span>
