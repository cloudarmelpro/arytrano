...
import { sendStudentWeeklyDigests } from '@/features/student-digest/services/send-student-weekly-digests'
...
export async function GET(request: Request) {
  ...
  try {
    const result = await sendStudentWeeklyDigests()
    return NextResponse.json({ ok: true, ...result })
  } catch (err) {
    Sentry.captureException(err, { tags: { cron: 'student-weekly-digest' } })
