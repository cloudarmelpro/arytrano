...
import { sweepOldPhoneOtps } from '@/features/phone-otp/server'
...
/**
 * Audit follow-up 2026-06-12 — daily sweep of PhoneOtp rows older
 * than 24h. The schema promised it ; this implements it.
 *
 * Scheduling : Vercel Cron daily (matches sweep-lapsed-leads et al.)
 */
...
export async function GET(request: Request) {
  ...
  if (!env.CRON_SECRET) {
    Sentry.captureMessage('Cron disabled (CRON_SECRET missing)', {
      level: 'error',
      tags: { cron: 'sweep-phone-otps', issue: 'disabled' },
    })
    return NextResponse.json({ ok: false, error: 'unauthorized' }, { status: 401 })
  }
  if (!bearerEquals(request.headers.get('authorization'), env.CRON_SECRET)) {
    Sentry.captureMessage('Cron auth failed (bad bearer)', {
      level: 'warning',
      tags: { cron: 'sweep-phone-otps', auth: 'failed' },
    })
    return NextResponse.json({ ok: false, error: 'unauthorized' }, { status: 401 })
  }

  try {
    const result = await sweepOldPhoneOtps()
    return NextResponse.json({ ok: true, ...result })
  } catch (err) {
    Sentry.captureException(err, { tags: { cron: 'sweep-phone-otps' } })
